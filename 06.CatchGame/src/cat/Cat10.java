...
/***
 * Cat10 클래스는 열 번째 고양이 캐릭터 "복실이"를 구현한 클래스입니다.  
...
public class Cat10 extends Base {
    ...
    public Cat10() {
        ...
        this.gender = "암컷";
        this.speed = 10;
        this.catchProbability = 10;  // 포획 확률 10%
    }

    /**
     * 복실이를 찾는 메서드입니다.  
     * 50% 확률로 복실이를 발견할 수 있으며, 탐색 결과를 콘솔에 출력합니다.
     *
     * @return true - 복실이를 발견함  
     *         false - 발견하지 못함      */
    @Override
     ...
     * - 아이템을 사용하지 않은 경우에는 10% 확률로 포획을 시도합니다.  
    ...
    public boolean catchCat(boolean item) {
        ...
        if (randNumber < this.catchProbability || item == true) {  // 확률이 catchProbability 이하일 때 포획 성공
            ...
            System.out.println(Config.GREEN + this.name + Config.RESET + "가(이) 간식을 보자마자 폴짝 뛰어들었다. 털이 복슬복슬~ 보기만 해도 따뜻해요.");
