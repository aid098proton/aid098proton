...
/***
 * Cat9 클래스는 아홉 번째 고양이 캐릭터 "칠복이"를 구현한 클래스입니다.  
...
public class Cat9 extends Base {
    ...
    public Cat9() {
        ...
        this.gender = "수컷";
        this.speed = 9;
        this.catchProbability = 20;  // 포획 확률 20%
    }

    /**
     * 칠복이를 찾는 메서드입니다.  
     * 50% 확률로 칠복이를 발견할 수 있으며, 탐색 결과를 콘솔에 출력합니다.
     *
     * @return true - 칠복이를 발견함  
     *         false - 발견하지 못함      */
    @Override
     ...
     * - 아이템을 사용하지 않은 경우에는 20% 확률로 포획을 시도합니다.  
    ...
    public boolean catchCat(boolean item) {
        ...
        if (randNumber < this.catchProbability || item == true) {  // 확률이 catchProbability 이하일 때 포획 성공
            ...
            System.out.println(Config.GREEN + this.name + Config.RESET + "가(이) 떨어진 간식이 자기 앞으로 굴러오자 덥석! 그 운으로 또 잡혔네요.");
